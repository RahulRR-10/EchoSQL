...
import { FaMicrophone, FaMicrophoneSlash, FaStar, FaRegStar } from "react-icons/fa";
...
// Wave component for the recording animation
const Wave = () => {
...
export default function VoiceInputBar({
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      // Clear bookmark status message on any key press
      if (bookmarkStatus) {
        setBookmarkStatus('');
      }
    };
  ...
  }, [inputText, onSend, bookmarkStatus]);

  // Handle bookmark toggle
  const handleBookmarkToggle = () => {
    if (!inputText.trim()) {
      setBookmarkStatus('Enter a query to bookmark');
      setTimeout(() => setBookmarkStatus(''), 2000);
      return;
    }

    if (isBookmarked(inputText.trim())) {
      setBookmarkStatus('Query already bookmarked');
      setTimeout(() => setBookmarkStatus(''), 2000);
      return;
    }

    const result = addBookmark(inputText.trim());
    if (result.success) {
      setBookmarkStatus('Query bookmarked!');
      setTimeout(() => setBookmarkStatus(''), 2000);
    } else {
      setBookmarkStatus(result.message || 'Failed to bookmark');
      setTimeout(() => setBookmarkStatus(''), 2000);
    }
  };

  // Handle bookmark selection
  const handleBookmarkSelect = (query) => {
    setInputText(query);
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        try {
          const res = await axios.post(
            "http://localhost:1111/speech-to-text",
            formData,
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto px-4">
      ...
      <div className="relative">
        {/* Bookmark Status Message */}
        {bookmarkStatus && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-full left-0 right-0 mb-2 text-center"
          >
            <span className="inline-block bg-[#1a2a2a] text-cyan-400 px-3 py-1 rounded text-sm border border-cyan-500/30">
              {bookmarkStatus}
            </span>
          </motion.div>
        )}
        ...
        <BookmarksDropdown
          ...
          onSelectQuery={handleBookmarkSelect}
        />
        ...
        <div className="flex gap-3 ">
          ...
          <motion.button
            ...
            className={`p-3.5 rounded-lg font-medium transition-all duration-300 
                       ...
                       ${isBookmarked(inputText.trim()) 
                         ? "bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30"
                         : "bg-gray-600/20 text-gray-400 hover:bg-gray-600/30"
                       }`}
            title={isBookmarked(inputText.trim()) ? "Already bookmarked" : "Bookmark this query"}
          >
            {isBookmarked(inputText.trim()) ? (
              <FaStar size={16} />
            ) : (
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            disabled={isLoading}
            className="p-3.5 rounded-lg font-medium bg-purple-600/20 text-purple-400 hover:bg-purple-600/30
                       transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            title="View bookmarked queries"
          >
            <FaStar size={16} />
          </motion.button>
